...
Ext.extend(Ext.ux.grid.GridSummary, Ext.util.Observable, {
    ...
    init : function(grid) {
        ...
        if (this.onlySeleccionModel) {
            this.sm.on({
            	selectionchange : this.refreshSummary,
            	scope: this
            });
        }	
            ...
            for (var i = 0, len = cfg.length; i < len; i++) { // loop through all columns in ColumnModel
                ...
                if (cf.summaryType) {
                    for (var j = 0, jlen = rs.length; j < jlen; j++) {
                        ...
                        var value = 0;
                        if (!this.onlySeleccionModel || cf.summaryType != 'sum' || this.sm.isSelected(r)) {
                        	value = r.get(cname);
                        }
                        data[cname] = Ext.ux.grid.GridSummary.Calculations[cf.summaryType](value, r, cname, data, j);
                    }
                }
            }
    ...
    getValueSummaryNode : function(id) {
    	var t = this.getSummaryNode();
    	return t.dom.childNodes[0].childNodes[0].childNodes[0].childNodes[id].textContent;
	},
	
	onlySeleccionModel : false
